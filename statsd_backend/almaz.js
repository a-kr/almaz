...
/*
 * Flush stats to almaz (http://github.com/Babazka/almaz).
 *
 * To enable this backend, include 'almaz' in the backends
 * configuration array:
 *
 *   backends: ['almaz']
 *
 ...
 *
 *   almazHost: Hostname of graphite server.
 *   almazPort: Port to contact graphite server at.
 *
 * The backend is shamelessly based on statsd's built-in 
 * Graphite backend. In fact, since almaz is protocol-compatible
 * with Graphite, the only change required was renaming of 
 * config option names, so that graphite and almaz backends
...
exports.init = function graphite_init(startup_time, config, events) {
  ...
  debug = config.debug;
  graphiteHost = config.almazHost;
  graphitePort = config.almazPort || 7701;
